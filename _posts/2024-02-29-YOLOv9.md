...
![alt text](assets/img/infobottleneck1.png)
...
![alt text](assets/img/infobottleneck2.png)
...
![alt text](assets/img/cspnet.png)

- Part 1 is sent through a series of transformations (e.g., convolutions, activation functions).

- Part 2 bypasses the transformations and retains the original information.	

The "partial" in cross-stage partial refers to how the network processes these split features. Instead of passing all feature maps through dense connections, only a portion of them go through the dense block. This significantly reduces computational complexity while maintaining network capacity.

The second part of GELAN is the Efficient Layer Aggregation Network or ELAN. ELAN was designed to solve a fundamental challenge in deep learning: how to effectively combine information from different layers of a neural network while keeping computational costs manageable. When researchers try to make neural networks deeper (what we call "model scaling"), they often hit a wall. After a certain depth, adding more layers doesn't help much, and sometimes it actually makes the network perform worse. For example, ResNet-152 uses three times more computing power than ResNet-50 but only improves accuracy by less than 1%. Even more striking, when ResNet grows to 200 layers, it performs worse than ResNet-152. Also - there's the information bottleneck principle at play again. In a traditional convolutional neural network, each layer processes information sequentially, passing it forward to the next layer - but this as we discussed above causes a lot of information loss due to information being bottlenecked. 
