...
## Zipf's Law and Stop Words Removal
...
Hence it is a good idea to trim out these stop words. There are several lists of stop words built into NLP libraries like `nltk` and `spacy` that can remove these stop word tokens from our corpus.
